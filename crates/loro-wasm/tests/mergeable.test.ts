...
describe("mergeable containers (WASM bindings)", () => {
  ...
  //
  // The six `getMergeable*` methods on `LoroMap` take a state-lock to register a side-table
  // entry, but do not emit a document-level event for that registration (registration is not
  // an op). Still, downstream mutations performed through the returned handle (e.g.
  // `counter.increment`, `tree.createNode`, list/text inserts) DO emit events. With an active
  // subscription on the parent, calling `getMergeable*` and then mutating must NOT leave any
  // events on the JS pending queue past the microtask boundary. If any `getMergeable*` ever
  // started enqueueing implicit events (today they don't), the binding decoration allowlist
  // in `crates/loro-wasm/index.ts` would need to grow to cover them.
  //
