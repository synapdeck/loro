...
use loro::{
    ContainerID, ContainerTrait, ContainerType, ExportMode, LoroDoc, ToJson, TreeParentId,
};
use serde_json::json;
...
/// Type mismatch through the public API: register one mergeable kind under "k",
/// then ask for another. The public `LoroMap::get_mergeable_*` wrappers must
/// surface the same `LoroError::ArgErr` that the underlying handler raises.
#[test]
#[parallel]
fn loro_map_get_mergeable_type_mismatch_through_public_api() {
    let d = doc(1);
    ...
    let err = root
        ...
        .expect_err("type mismatch through the public API must error");
    let msg = format!("{err:?}");
    assert!(
        msg.contains("Mergeable key") && msg.contains("Map") && msg.contains("Text"),
        "public-API error must describe the mergeable type mismatch; got {msg}"
    );
}
